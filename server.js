...
// --- piste lumineuse façon dancefloor disco : des cases éteintes qui
// s'allument de temps en temps, et rapportent 1 à 5 pièces à qui marche
// dessus (elle s'éteint alors, jusqu'à ce qu'une autre s'allume ailleurs). ---
const DISCO_GRID_COLS = 8;
const DISCO_GRID_ROWS = 3;
const DISCO_TILE_INTERVAL_MIN_MS = 1200;
const DISCO_TILE_INTERVAL_MAX_MS = 2600;
const DISCO_TILE_MAX_LIT = 4;
...
// Construit une grille de cases disco réparties sur la même zone atteignable
// que les objets (cf. randomFloorPosition), toutes éteintes au départ.
function buildDiscoGrid(room) {
  const xMin = 0.12, xMax = 0.88;
  const yMin = Math.min(0.84, room.floorMinY + 0.05), yMax = 0.88;
  const tiles = [];
  let id = 1;
  for (let r = 0; r < DISCO_GRID_ROWS; r++) {
    for (let c = 0; c < DISCO_GRID_COLS; c++) {
      ...
      tiles.push({ id: id++, x, y, lit: false });
    }
  }
...
// Allume une case éteinte au hasard, tant qu'il n'y en a pas déjà trop
// d'allumées en même temps, puis se reprogramme pour la prochaine fois.
...
function startRoundCountdown(room, roomId) {
  ...
  io.to(roomId).emit('round-state', { active: false, items: [], discoTiles: [], countdownEndAt: anchorAt });
...
function endRound(room, roomId) {
  ...
  io.to(roomId).emit('round-state', { active: false, items: [], discoTiles: [], countdownEndAt: null });
}
