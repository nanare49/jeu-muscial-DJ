...
function settleDjTurn(room, roomId) {
  ...
  const DJ_TURN_XP = 10, DJ_TURN_COINS = 5;
...
// Choisit une position aléatoire dans la zone de la piste (sous la scène, à
// l'intérieur des bornes où les festivaliers peuvent marcher côté client).
function randomFloorPosition() {
  return {
    x: 0.08 + Math.random() * 0.84,
    y: 0.5 + Math.random() * 0.36
  };
}
...
function startRoundCountdown(room, roomId) {
  ...
  io.to(roomId).emit('round-state', { active: false, items: [], countdownEndAt: anchorAt });
...
function activateRound(room, roomId) {
  ...
  io.to(roomId).emit('round-state', { active: true, items: Object.values(room.round.items), countdownEndAt: null });
}
...
function endRound(room, roomId) {
  ...
  io.to(roomId).emit('round-state', { active: false, items: [], countdownEndAt: null });
}
